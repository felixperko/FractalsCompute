...
public class InputScannerThread extends AbstractFractalsThread {
	...
	public void run() {
		...
		while (getLifeCycleState() != LifeCycleState.STOPPED) {
			while (!scanner.hasNext() && getLifeCycleState() != LifeCycleState.STOPPED)
				try {
					Thread.sleep(1);
